...
class _ExpenseReportScreenState extends ConsumerState<ExpenseReportScreen> {
  ...
  void _showExpenseTypeSelectionDialog(
      String expenseType, String label, IconData icon) {
  ...
  Future<void> _downloadAndShareCSV() async {
    try {
      ...
      final result = await ExpenseAPI.generateExpenseExcel(
        serviceType: type,
  ...
  Future<void> _downloadCSVFile() async {
    try {
      ...
      if (Platform.isAndroid || Platform.isIOS) {
        await _saveMobileCSV(bytes, fileName);
      } else {
  ...
  Future<void> downloadAllExpensesCSV() async {
    try {
      ...
      // FORCE all expenses
      final result = await ExpenseAPI.generateExpenseCSV(
        serviceType: type,
  ...
  Future<void> _saveMobileCSV(Uint8List bytes, String fileName) async {
    try {
  ...
  Future<void> _saveDesktopCSV(Uint8List bytes) async {
    final fileName =
        'expense_report_${DateFormat('yyyyMMdd').format(_selectedStartDate!)}_${DateFormat('yyyyMMdd').format(_selectedEndDate!)}.csv';
  ...
  Color _categoryColor(BuildContext context, String value) {
    final colorScheme = Theme.of(context).colorScheme;
    switch (value) {
      case 'material_tools':
                  ...
                  child: GridView.count(
                    ...
                    children: expenseTypes.map((type) {
                      return _expenseTypeCard(
                        ...
                        label: type['label'],
                        icon: type['icon'],
                        color: _categoryColor(context, type['value']),
                      );
                    }).toList(),
                  ),
                ),
